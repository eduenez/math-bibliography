...
Key-rename map
--------------
If rekey.json exists alongside this script (in the math-bibliography directory),
it is loaded automatically and applied to every \cite{} command during migration.
rekey.json is generated by generate_rekey_map.py and maps every old citation key
(from biblioteca.bib, iovino.bib, or earlier versions of references.bib) to its
canonical Author-Author:YYYY key in the current references.bib.

To override or suppress the default map use --map or --no-map respectively.
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help='JSON file {"old_key": "new_key", ...} for renaming citation keys.'
             " Overrides the automatic rekey.json lookup.",
    )
    parser.add_argument(
        "--no-map",
        action="store_true",
        help="Disable automatic loading of rekey.json (skip cite-key renames).",
    )
    parser.add_argument(
        "--print-keys",
        action="store_true",
    ...
    elif not args.no_map and REKEY_JSON.exists():
        with open(REKEY_JSON, encoding="utf-8") as f:
            rekey = json.load(f)
        print(f"Auto-loaded {len(rekey)} key rename(s) from {REKEY_JSON}")
